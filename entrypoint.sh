...
if "${INPUT_UPDATE_DEPENDENCY}" && [ -e composer.json ]; then
  printf '\033[33m%s\033[m\n' "[${ACTION_NAME}] Update dependencies as 'composer.json' exists."
  COMPOSER_MEMORY_LIMIT=-1 $(which composer) update
  COMPOSER_STATUS=$?
else
  if "${INPUT_UPDATE_DEPENDENCY}"; then
    printf '\033[33m%s\033[m\n' "[${ACTION_NAME}] Dependencies are not updated because 'composer.json' does not exist."
  else
    printf '\033[33m%s\033[m\n' "[${ACTION_NAME}] Dependencies will not be updated as the 'update_dependency' option is false."
  fi
  COMPOSER_STATUS=0
fi
...
if [ ${COMPOSER_STATUS} -ne 0 ]; then
  printf '\033[31m%s\033[m\n' "[${ACTION_NAME}] Failed composer update."
  exit 1
elif [ ${PHPCS_STATUS} -ne 0 ] || [ ${PHPMD_STATUS} -ne 0 ] || [ ${PHINDER_STATUS} -ne 0 ]; then
  printf '\033[31m%s\033[m\n' "[${ACTION_NAME}] Failed any analyse. Please fix the code."
  exit 1
else
  printf '\033[32m%s\033[m\n' "[${ACTION_NAME}] Successful all analyse."
  exit 0
fi
